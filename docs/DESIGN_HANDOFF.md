...
1. **실재 콘텐츠 = 소방관계법규 1과목만.** 목업의 4과목(소방학개론·행정법총론·소방전술)·done 카운트·진행률(%)은 **데모 더미다. 하드코딩 금지.** 과목 화면은 실제 과목만 렌더. 진행률은 실제 데이터(Isar) 있으면 표시, 없으면 생략(가짜 수치 만들지 말 것). **문항 수도 하드코딩 금지** — "25문항" 등 상수 대신 실제 로드된 문항 수를 사용(콘텐츠 뱅크가 늘어남). 기존 `home_page.dart`의 "25문항 순서대로"·"25문항" 카피를 동적 count로 교체.
